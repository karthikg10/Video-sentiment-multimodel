...
class MELDDataset(Dataset):
    def __init__(self, csv_path, video_dir):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained('bert-base-uncased', use_authtoken=False)
    ...
    def load_video_frames(self, video_path):
        ...
        try:
            ...
            while len(frames) < 30 and cap.isOpened():
                ...
                frame = cv2.resize(frame, (224, 224))  # Resize frame to a fixed size (e.g., 224x224)
        ...
        if len(frames) < 30:
            frames += [np.zeroes_like(frames[0])] * (30 - len(frames))
        else:
            frames = frames[:30]
        # Before permute: [frames, height, width, channels]
        # After permute: [frames, channels, height, width]
        return torch.FloatTensor(np.array(frames)).permute(0, 3, 1, 2)  # Convert to tensor and change shape to (frames, channels, height, width)
